...
3. ✅ **Fase 3′ — Working root fuera del árbol servido (HECHA).** `ENGINE_OUTPUT_ROOT=../../.data/engine` (ignorado) contiene la salida COMPLETA (1360 MB). `serveEngineAssets()` ahora sirve en dev desde **dos raíces en orden** — working root primero (lo que necesita el cockpit), publicado después — conservando el chequeo de path-traversal. Se quitó el parche `watch.ignored` (el motor ya no escribe en `public/`). **Resultado medido: el store en `public/` pasó de 1360 MB a 63 MB (42 archivos)**; el deploy deja de arrastrar 1,3 GB. Verificado end-to-end: un asset publicado responde 200 desde `public/` y una lámina NO publicada del AI-300 responde 200 desde `.data/engine/`.
4. ⏳ **Archivar la cubeta D** (~1,3 GB, ahora en `.data/engine/`). **Precisión importante:** borrar del working tree **no borra de la historia** — los objetos LFS siguen recuperables con `git checkout <commit> -- <ruta>`, así que **git sigue siendo el archivo** de los 658 MB que estaban trackeados. Los otros ~700 MB nunca estuvieron en git y hoy sólo viven en `.data/`. **Condición dura: mientras no exista un archivo externo, la Fase 5 (reescribir historia) NO se puede hacer** — sería la única forma de perder de verdad esas láminas.
